...
interface GarbageHotspotSurveyModalProps {
  open: boolean;
  onClose: () => void;
  onSubmit: (data: GarbageHotspotData) => void;
}
...
const ISSUE_TYPES = [
  { value: 'illegal_dumping', label: 'Illegal dumping site' },
  { value: 'missed_collection', label: 'Missed garbage collection' },
  { value: 'overflowing_bins', label: 'Overflowing public bins' },
  { value: 'no_bins', label: 'No bins in the area' },
];
...
export const GarbageHotspotSurveyModal: React.FC<GarbageHotspotSurveyModalProps> = ({
  open,
  onClose,
  onSubmit,
}) => {
  ...
  const [reportCount] = useState(178); // Mock ward report count
  ...
  const handleSubmit = () => {
    if (!pinLocation) {
      toast.error('Please drop a pin on the map to mark the garbage hotspot.');
      return;
    }
    if (!issueType) {
      toast.error('Please select the type of waste issue.');
      return;
    }

    const data: GarbageHotspotData = {
      surveyId: 'survey_003',
    ...
    onSubmit(data);
    setIsSubmitted(true);
    toast.success('Your garbage hotspot has been recorded.');
  };
  ...
  return (
    <Dialog open={open} onOpenChange={(isOpen) => !isOpen && onClose()}>
      <DialogContent 
        ...
        aria-describedby="garbage-survey-description"
      >
        <DialogHeader className="space-y-1">
          <div className="flex items-center gap-2">
            <Trash2 className="w-5 h-5 text-warning" aria-hidden="true" />
            <DialogTitle className="text-xl font-bold text-foreground">
              Garbage Hotspot Mapping
            </DialogTitle>
          </div>
          <DialogDescription id="garbage-survey-description" className="text-muted-foreground">
            Help us identify areas with waste management issues in your neighborhood.
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="mt-4 space-y-5">
          ...
          <p className="font-semibold text-foreground">
            Where are the garbage problem areas in your ward?
          </p>
          ...
          <div className="space-y-3">
            <Label>Type of waste issue</Label>
            <RadioGroup
              value={issueType}
            ...
            >
              {ISSUE_TYPES.map((type) => (
                ...
                >
                  ...
                  <Label htmlFor={type.value} className="flex-1 cursor-pointer text-foreground">
                    {type.label}
                  </Label>
                </div>
              ))}
          ...
          <div className="space-y-2">
            <Label className="flex items-center gap-2">
              ...
              Drop a pin on the problem area
            </Label>
            ...
            <div 
              ...
              style={{ height: '250px' }}
              role="application"
              aria-label="Interactive map for selecting garbage hotspot location"
            >
              <MapContainer
                center={mapCenter}
                zoom={14}
          ...
          {/* Description */}
          <div className="space-y-2">
            <Label htmlFor="garbage-description">
              Describe the issue <span className="text-muted-foreground">(optional)</span>
            </Label>
            <Textarea
              id="garbage-description"
              value={description}
              onChange={(e) => setDescription(e.target.value)}
              placeholder="E.g., Garbage piled up near the stage, overflowing skip bin at the market, illegal dumping behind the shops..."
          ...
          <div className="flex gap-3 pt-4 border-t border-border">
            ...
            <Button
              ...
              disabled={!pinLocation || !issueType}
